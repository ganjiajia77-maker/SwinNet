...
parser.add_argument(
    ...
    action='store_true',
    help='keep structure gate and add F + gamma*mean(C_k*shift(F))',
)
parser.add_argument(
    '--enable_sc_graph_diffusion',
    action='store_true',
    help='keep structure gate; add F + gamma*(A@F) with A=C*(1+alpha*S_i*S_j), no D',
)
parser.add_argument(
...
if sum(
    (
        args.enable_graph_diffusion,
        args.enable_simple_c_diffusion,
        args.enable_sc_graph_diffusion,
    )
) > 1:
    parser.error(
        "Use only one of --enable_graph_diffusion, "
        "--enable_simple_c_diffusion, or --enable_sc_graph_diffusion."
    )
...
if args.enable_sc_graph_diffusion and not _cli_has("--structure_profile"):
    args.structure_profile = STRUCTURE_PROFILE_STAGE23_BOUNDARY_0626
if args.enable_sc_graph_diffusion and not args.resume:
    args.resume = DEFAULT_GRAPH_DIFFUSION_BASE_CHECKPOINT
    print(
        "[INFO] SC graph diffusion base checkpoint: "
        f"{DEFAULT_GRAPH_DIFFUSION_BASE_CHECKPOINT}",
        flush=True,
    )
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if args.enable_graph_diffusion:
                ...
                start_epoch = 0
                print(
                    "[INFO] SC graph diffusion warm-start: reset start_epoch to 0 "
                    f"(loaded weights from checkpoint epoch={checkpoint.get('epoch', '?')})",
                    flush=True,
                )
            elif (
