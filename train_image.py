            ...
            for image_index in range(full_images.shape[0]):
                ...
                for top in positions(height, tile_size, stride):
                    for left in positions(width, tile_size, stride):
                        ...
                        logit_canvas[:, :, top:top + tile_height, left:left + tile_width] += surface_logits
                        if skeleton_logits is not None:
                            skeleton_logit_canvas[:, :, top:top + tile_height, left:left + tile_width] += skeleton_logits
                        weight_canvas[:, :, top:top + tile_height, left:left + tile_width] += 1.0
