...
def build_feature_scores(z_struct, random_seed=1234):
    ...
    return {
        "feature_norm": minmax_normalize(norm_score),
        "feature_entropy": minmax_normalize(entropy),
        "random_projection": minmax_normalize(projection),
    }
...
def main():
    ...
    print("[feature-anchor] scores: feature_norm, feature_entropy, random_projection", flush=True)
    ...
    with torch.no_grad():
        for batch_index, batch in enumerate(loader):
            ...
            scores = build_feature_scores(z_struct, random_seed=args.random_seed)
    ...
    for mode in ("feature_norm", "feature_entropy", "random_projection"):
